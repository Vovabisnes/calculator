...
class ValidatorTest {
    ...
    class parseLineTests {
        static Stream<Arguments> dataFor_Should_ReturnCorrectArrayList_When_CorrectInput() {
            return Stream.of(
                    Arguments.of("4 * 0+3", List.of("4", "*", "0", "+", "3")),
                    Arguments.of("2/4-1", List.of("2", "/", "4", "-", "1")),
                    Arguments.of("50/0.1", List.of("50", "/", "0.1")),
                    Arguments.of("4", List.of("4")));
        }

        @ParameterizedTest
        @MethodSource({"dataFor_Should_ReturnCorrectArrayList_When_CorrectInput"})
        void should_ReturnCorrectArrayList_When_CorrectInput(String line, List<String> excepted) throws Exception {
            //Given

            //When
            List<String> actual = validator.parseLine(line);
